...
const CryptoPayment = ({ user }) => {
    ...
    useEffect(() => {
        const checkPendingTransaction = async () => {
            ...
            try {
                const response = await axios.get(`http://localhost:5000/api/check-pending-transaction/${email}`);
    ...
    const handleCancelTransaction = async () => {
        try {
            const response = await axios.post('http://localhost:5000/api/cancel-transaction', {
                walletAddress,
        ...
        if (walletAddress && transactionStatus === 'pending' && !isTransactionCanceled) {
            const pollForPayment = async () => {
                try {
                    ...
                    const response = await axios.get(`http://localhost:5000/api/check-payment/${walletAddress}/${cryptoAmount}`);
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            const response = await axios.post('http://localhost:5000/api/payment', { amount: usdAmount, userEmail: email });
